...
-- module.
genVerilogTop :: Opts    -- ^ Code generator options
              -> Netlist -- ^ Blarney module
              -> String  -- ^ Top-level module name
              -> String  -- ^ Output directory
              -> IO ()
genVerilogTop opts nl mod dir =
  ...
  where simCode = unlines . filter (/= "") $ [
            "// Generated by Blarney"
          , "#include <verilated.h>"
          , ifTrace "#include <verilated_vcd_c.h>"
          , "#include \"V" ++ mod ++ ".h\""
          , "V" ++ mod ++ " *top;"
          ...
          , "  top = new V" ++ mod ++ ";"
          , ifTrace "  Verilated::traceEverOn(true);"
          , ifTrace "  VerilatedVcdC* trc = new VerilatedVcdC;"
          , ifTrace "  top->trace(trc, 99);"
          , ifTrace "  trc->open(\"trace.vcd\");"
          , "  while (!Verilated::gotFinish()) {"
          , "    top->clock = 0; top->eval();"
          , ifTrace "    trc->dump((2*main_time)+0);"
          , "    top->clock = 1; top->eval();"
          , ifTrace "    trc->dump((2*main_time)+1);"
          , "    main_time++;"
          , "  }"
          , ifTrace "  trc->close();"
        ...
        ifTrace s = if optEnableVerilatorTrace opts then s else ""
