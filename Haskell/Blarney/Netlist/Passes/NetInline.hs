...
netInputInline :: MNetlistPassCtxtRef s -> NetCounts s -> NetInput
               -> ST s (NetInput, Bool)
netInputInline ctxtRef nc inpt@(InputWire (instId, _)) = do
  -- read ref count for our referenced 'Net'
  cnt <- readArray nc instId
  -- read netPrim and netInputs for our referenced 'Net'
  Net{ netPrim = prim, netInputs = inpts } <- readNet instId ctxtRef
  ...
  if cnt == 1 && canInline prim then do
    (inpts', changes) <- unzip <$> mapM
      (\x -> if canInlineInput prim then
                do (x', _) <- netInputInline ctxtRef nc x
                   return (x', True)
             else return (x, False)) inpts
    return (InputTree prim inpts', or changes)
  else return (inpt, False)
netInputInline ctxtRef nc (InputTree prim inpts) = do
  (inpts', changes) <- unzip <$> mapM (netInputInline ctxtRef nc) inpts
  return $ (InputTree prim inpts', or changes)

-- | Single-reference 'Net' inlining pass
singleRefNetInline :: MNetlistPass s Bool
singleRefNetInline ctxtRef = do
  ...
  forM_ [(a,b) | x@(a, Just b) <- pairs, canInlineInput (netPrim b)] $
    \(idx, net) -> do
      (netInputs', changes) <- unzip <$> mapM (netInputInline ctxtRef refCounts)
                                              (netInputs net)
