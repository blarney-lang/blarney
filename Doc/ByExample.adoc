...
values to increment, and display the results.
...
As the <<Bits>> class does <<Bits, not support Haskell sum types>>, we
need a different way to capture enum types that will be stored
as circuit values.  We suggest that enum types such as
