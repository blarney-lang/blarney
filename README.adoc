...
first-class actions and method-based interfaces.  Applications of
Blarney include https://github.com/CTSRD-CHERI/SIMTight[SIMTight] (a
CHERI-enabled RISC-V GPGPU) and
https://github.com/blarney-lang/actora[Actora] (a stack machine with
a compiler for an Erlang-like language).  Below, we introduce the
...
You should see the same output as before.  Using Verilator is the
recommended approach for simulating Blarney designs.
...
function to produce data at a variable rate.  Isn't it therefore
...
consume before producing data; but when a function returns a source as
output, it does not.  This can be the difference between requiring or
not requiring a buffer between producer and consumer.  Often, we don't
mind buffering (it's good for Fmax) so our convention is to use the
`Stream` type whenever possible.
