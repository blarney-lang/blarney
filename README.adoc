...
using the same bit rotation strategy as the splitting stage. We
...
farm example].  It's biggest weakness is that it must return responses
in order, even if later responses become available before earlier
ones.  An interesting exercise might be to develop a server farm that
