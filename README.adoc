...
To use Blarney, we'll need Verilator (e.g. `sudo apt install
verilator` on Ubuntu) and GHC 9.2.1 or later (e.g. using
[ghcup](https://www.haskell.org/ghcup/)).

After cloning the Blarney, we set the `BLARNEY_ROOT` environment
...
value of a register; and `when` allows conditional actions to be
introduced.  In addition to `when`, we can also use if-then-else in an
