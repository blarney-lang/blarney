...
output, it does not.  This can be the difference between
requiring or not requiring a buffer between producer and consumer.
Often, we don't mind buffering (it's good for Fmax) and in such cases
our convention is to use the `Stream` type.
